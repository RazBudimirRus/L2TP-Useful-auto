...
### Новые возможности автозапуска:
- **Задержка запуска:** 15 секунд после загрузки системы
- **Повторные попытки:** до 3 попыток с интервалом 30 секунд
- **Проверка сервера:** автоматическая проверка доступности L2TP сервера
- **Детальное логирование:** полные отчеты о всех операциях

### Проверка доступности сервера:
```bash
# Скрипт автоматически проверяет:
ping -c 10 main.razbudimir.com
ping -c 10 78.107.255.229
```
